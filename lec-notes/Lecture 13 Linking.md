...
从本节开始，将从**程序如何与硬件交互**转变为**程序如何与特定系统软件中软件交互**。从链接开始，弄清系统如何构建程序。
...
- **符号解析**
  - **链接过程中，链接器将每个符号引用与一个符号定义相关联**。每个符号对应一个函数、一个全局变量或一个静态变量（即用 `static` 属性声明的 C 变量）。
  - 符号定义存储在目标文件中的符号表中：符号表是一个结构数组，每个结构数据中包含着符号的名称，大小以及位置。
...
- **可重定位目标文件（Relocateable object file）**  
  包含二进制代码与数据，可在链接时与其他同类型文件合并，生成可执行目标文件（通常以 `.o` 或 `.obj` 为扩展名）。无法以任何形式直接加载到内存中，实际使用前需要链接器对齐进行操作。
...
- 现代 **x86-64 Linux** 系统和 **Unix** 系统使用 可执行可链接格式（**Executable and Linkable Format， ELF**）是 .so 文件、.out文件、.o文件的统一格式
...
**ELF头**：位于文件开头。
  - 一个16字节的序列，描述了生成该文件的**系统的字长和字节顺序**。
  - 剩余部分包含**让链接器能够解析和解释该目标文件**的信息，如：ELF头的大小、目标文件类型、机器类型、节头部表在文件中的偏移量以及节头部表条目的大小和数量。
